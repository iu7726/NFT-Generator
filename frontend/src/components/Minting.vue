...
const mint = async (nftAddress: string) => {
  const signer = await connect();
  const nft = new ethers.Contract(nftAddress, store.erc721ABI, signer);
  ...
  try {
    if (nftInfo.wl.active && now >= nftInfo.wl.startTime && now <= nftInfo.wl.endTime) {
      console.log(nftInfo.wl.cost);
      await nft.buyWlNft(nftInfo.amount, {
        from: await signer.getAddress(),
        value: nftInfo.wl.cost.mul(BigNumber.from(nftInfo.amount))
      })
    } else if (nftInfo.pub.active && now >= nftInfo.pub.startTime && now <= nftInfo.pub.endTime) {
      await nft.buyPubNft(nftInfo.amount, {
        from: await signer.getAddress(),
        value: nftInfo.pub.cost.mul(BigNumber.from(nftInfo.amount))
      })
    }
  } catch (error: any) {
  ...
  <div class="greetings">
    ...
    >
        ...
        <div class="nft-mint">
          ...
          <input type="number" v-model="nft.amount"/>
          <button 
            class="btn-mint"
            @click="mint(nft.address)"
          >mint</button>
        </div>
    </div>
